...
rownames(cluster_mat) <- wrap_text(cluster_pathway_data$Description, width = 50)
...
network_data <- convergent_top %>%
  ...
  mutate(
    Description_short = ifelse(nchar(Description) > 30,
                              paste0(wrap_text(Description, width = 30), "..."),
                              Description),
...
p12 <- ggplot(data = NULL) +
  ...
  annotate("text", x = 0.5, y = 0.5, 
           label = paste0(wrap_text(key_stats$top_convergent, width = 60), "..."),
           size = 5) +
