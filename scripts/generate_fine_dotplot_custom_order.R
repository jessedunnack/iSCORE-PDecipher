...
# GENERATE FINE CLUSTER DOTPLOT WITH CUSTOM USER-SPECIFIED ORDERING
# Orders fine clusters based on custom coarse parent order
...
cat("=================================================================\n")
cat("FINE DOTPLOT WITH CUSTOM USER-SPECIFIED ORDERING\n")
cat("=================================================================\n\n")
...
# 2. Define custom coarse cluster ordering
cat("\n2. Setting up custom user-specified ordering...\n")

# User's preferred coarse cluster order (bottom to top)
custom_coarse_order <- c(1, 4, 8, 3, 5, 9, 12, 7, 6, 10, 13, 11, 2, 14, 0)

# Define groups for reference
custom_groups <- list(
...
for (i in seq_along(custom_coarse_order)) {
  coarse_cl <- custom_coarse_order[i]
...
for (coarse_cl in custom_coarse_order) {
...
p_enhanced <- ggplot(plot_data, aes(x = features.plot, y = id)) +
  ...
  labs(
    ...
    title = "Fine Cluster Markers - Custom Order",
...
# Also add thicker lines between major groups
group_boundaries_y <- numeric()
current_pos <- 0
for (group_name in names(custom_groups)) {
  group_clusters <- custom_groups[[group_name]]
  n_fine_in_group <- sum(fine_to_coarse$coarse_cluster %in% group_clusters)
  if (n_fine_in_group > 0) {
    current_pos <- current_pos + n_fine_in_group
    if (current_pos < length(fine_cluster_order)) {
      group_boundaries_y <- c(group_boundaries_y, current_pos)
    }
  }
}

for (boundary in group_boundaries_y) {
  p_enhanced <- p_enhanced +
    geom_hline(yintercept = boundary + 0.5, 
...
pdf("results/dotplots/dotplot_fine_clusters_custom_order.pdf", width = 18, height = 14)
print(p_enhanced)
dev.off()
cat("  - Saved PDF: results/dotplots/dotplot_fine_clusters_custom_order.pdf\n")

png("results/dotplots/dotplot_fine_clusters_custom_order.png", width = 1800, height = 1400, res = 150)
print(p_enhanced)
dev.off()
cat("  - Saved PNG: results/dotplots/dotplot_fine_clusters_custom_order.png\n")
...
fine_order_summary$group <- NA
for (group_name in names(custom_groups)) {
  clusters_in_group <- custom_groups[[group_name]]
  fine_order_summary$group[fine_order_summary$coarse_cluster %in% clusters_in_group] <- group_name
}

write.csv(fine_order_summary, 
          "results/dotplots/fine_custom_order_summary.csv", 
          row.names = FALSE)

# Print summary
group_summary <- fine_order_summary %>%
  group_by(group) %>%
  summarise(
    n_fine_clusters = n(),
  ...
  mutate(
    group = factor(group, levels = names(custom_groups))
  ) %>%
  arrange(group)

cat("\nFine clusters by custom groups:\n")
print(group_summary)

cat("\n=== CUSTOM ORDER FINE DOTPLOT COMPLETE ===\n")
cat("Features:\n")
cat("- Fine clusters ordered by custom coarse parent order\n")
cat("- Grouped by user-specified cell type categories\n")
cat("- Genes clustered within each coarse parent group\n")
cat("- Visual separators between cell types and major groups\n")
