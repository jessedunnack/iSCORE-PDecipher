...
# RECLUSTER AND INVESTIGATE
# Re-run clustering with correct resolutions, then investigate
...
seurat_obj$seurat_clusters_coarse_new <- Idents(seurat_obj)
n_coarse <- length(unique(seurat_obj$seurat_clusters_coarse_new))
...
seurat_obj$seurat_clusters_fine_new <- Idents(seurat_obj)
n_fine <- length(unique(seurat_obj$seurat_clusters_fine_new))
...
check_gene_expression <- function(seurat_obj, genes, cluster_col = "seurat_clusters_coarse_new") {
...
cat("\n5. Quick marker check for new coarse clusters...\n")

# Check TH expression
th_expr <- check_gene_expression(seurat_obj, "TH", "seurat_clusters_coarse_new")
if (!is.null(th_expr)) {
  cat("\nTH expression in new coarse clusters:\n")
  th_expr <- th_expr %>% 
...
coarse_clusters <- sort(unique(as.numeric(as.character(seurat_obj$seurat_clusters_coarse_new))))

for (cl in coarse_clusters) {
  cat(sprintf("\nCluster %d (n=%d cells):\n", 
              cl, 
              sum(seurat_obj$seurat_clusters_coarse_new == as.character(cl))))
  
  for (set_name in names(key_markers)) {
    expr_data <- check_gene_expression(seurat_obj, key_markers[[set_name]], "seurat_clusters_coarse_new")
...
# Create mapping between old and new clusters
cat("\n\n7. Creating cluster mapping...\n")
if ("seurat_clusters_coarse" %in% names(seurat_obj@meta.data)) {
  mapping <- table(
    Old_Coarse = seurat_obj$seurat_clusters_coarse,
    New_Coarse = seurat_obj$seurat_clusters_coarse_new
  )
  cat("\nMapping between old and new coarse clusters:\n")
  print(mapping)
}

cat("\n\n=== RECLUSTERING COMPLETE ===\n")
cat("New coarse clusters:", n_coarse, "\n")
cat("New fine clusters:", n_fine, "\n")
cat("\nNext steps:\n")
cat("1. Run comprehensive marker analysis on new clusters\n")
