...
# Convert the HTML quickstart guide to PDF
# This script provides multiple options depending on available packages

library(rmarkdown)

# Option 1: Using rmarkdown (most reliable)
convert_with_rmarkdown <- function() {
  cat("🔄 Converting using rmarkdown::render()...\n")
  
  # Create a simple Rmd file that includes the HTML content
  rmd_content <- '---
title: "iSCORE-PDecipher Quick Start Guide"
output: 
  pdf_document:
    toc: true
    toc_depth: 2
...
# iSCORE-PDecipher Quick Start Guide
...
**Happy exploring!** 🧬 This tool contains 767,337 enrichment terms from comprehensive PD research - there\'s a lot to discover!'
  ...
  # Write the Rmd file
  writeLines(rmd_content, "LABMATE_QUICKSTART.Rmd")
  
  # Render to PDF
  rmarkdown::render("LABMATE_QUICKSTART.Rmd", 
                    output_file = "LABMATE_QUICKSTART.pdf",
                    quiet = FALSE)
  
  cat("✅ PDF created: LABMATE_QUICKSTART.pdf\n")
  
  # Clean up
  if (file.exists("LABMATE_QUICKSTART.Rmd")) {
    file.remove("LABMATE_QUICKSTART.Rmd")
  }
}

# Option 2: Using pagedown (if available)
convert_with_pagedown <- function() {
  if (!requireNamespace("pagedown", quietly = TRUE)) {
    cat("📦 Installing pagedown...\n")
    install.packages("pagedown")
  }
  
  cat("🔄 Converting using pagedown::chrome_print()...\n")
  pagedown::chrome_print("LABMATE_QUICKSTART.html", 
                         output = "LABMATE_QUICKSTART.pdf")
  cat("✅ PDF created: LABMATE_QUICKSTART.pdf\n")
}

# Main function
main <- function() {
  cat("🚀 iSCORE-PDecipher Quickstart Guide PDF Converter\n")
  cat("================================================\n\n")
  
  # Check current directory
  if (!file.exists("LABMATE_QUICKSTART.html")) {
    cat("❌ Error: LABMATE_QUICKSTART.html not found in current directory\n")
    cat("   Please run this script from the directory containing the HTML file\n")
    return(invisible(FALSE))
  }
  
  # Try conversion methods in order of preference
  tryCatch({
    convert_with_rmarkdown()
    cat("\n🎉 Success! Your PDF guide is ready for sharing with labmates.\n")
    cat("📄 File created: LABMATE_QUICKSTART.pdf\n")
    return(invisible(TRUE))
  }, error = function(e) {
    cat("⚠️  rmarkdown method failed:", e$message, "\n")
    
    tryCatch({
      convert_with_pagedown()
      cat("\n🎉 Success! Your PDF guide is ready for sharing with labmates.\n")
      cat("📄 File created: LABMATE_QUICKSTART.pdf\n")
      return(invisible(TRUE))
    }, error = function(e2) {
      cat("❌ Both conversion methods failed.\n")
      cat("📝 Alternative: Open LABMATE_QUICKSTART.html in your browser and print to PDF\n")
      cat("\nError details:\n")
      cat("- rmarkdown:", e$message, "\n")
      cat("- pagedown:", e2$message, "\n")
      return(invisible(FALSE))
    })
  })
}
