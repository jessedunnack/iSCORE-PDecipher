...
p1 <- mast_top %>%
  ...
  mutate(
    ...
    Description_short = truncate_for_bars(Description, source_col = enrichment_type),
...
top3_data$Pathway_short <- wrap_text(top3_data$Pathway, width = 30)

p4 <- ggplot(top3_data, aes(x = Category, y = Significance, fill = Category)) +
  geom_bar(stat = "identity") +
  geom_label(aes(label = Pathway_short, y = Significance/2),
             fill = 'white', color = 'black', alpha = 0.9,
             label.r = unit(0.25, 'lines'), label.padding = unit(0.5, 'lines'),
