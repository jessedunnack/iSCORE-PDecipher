...
ui <- fluidPage(
  ...
  tags$head(
    ...
    tags$link(rel = "stylesheet", type = "text/css", href = "nested_tabs.css"),
  ...
  div(class = "content-area",
    ...
    div(class = "sidebar-fixed",
      ...
      div(class = "sidebar-content",
        ...
        # Data Selection
        h5(icon("sliders-h"), "Data Selection", style = "margin-bottom: 15px; font-weight: bold; color: #337ab7;"),
      ...
      h5(icon("cog"), "Analysis Settings", style = "margin-bottom: 15px; font-weight: bold; color: #337ab7;"),
    ...
    div(class = "main-content",
      ...
      div(class = "main-sections",
        ...
        tabPanel(
          "DE Genes",
          icon = icon("dna"),
          value = "de_genes_section",
          br(),
          div(class = "section-header",
            h2("Differential Expression Gene Analysis"),
          ...
          div(class = "export-section",
            h4(icon("download"), "Export DE Gene Results"),
            p("Download your differential expression analysis results in various formats."),
            actionButton("export_de_genes", "Go to Export Options", class = "btn-info")
          )
        ),
        ...
        tabPanel(
          ...
          div(class = "export-section",
            h4(icon("download"), "Export Enrichment Results"),
...
server <- function(input, output, session) {
  ...
  output$section_indicator <- renderUI({
    current_section <- input$main_sections
    
    if (is.null(current_section)) return(NULL)
    
    section_info <- switch(current_section,
      "overview" = list(
        icon = icon("home"),
        text = "Overview applies to both sections",
        color = "#28a745"
      ),
      "de_genes_section" = list(
        icon = icon("dna"),
        text = "Settings apply to DE Genes analysis",
        color = "#3c8dbc"
      ),
      "enrichment_section" = list(
        icon = icon("chart-bar"),
        text = "Settings apply to Enrichment analysis",
        color = "#f39c12"
      ),
      "export" = list(
        icon = icon("download"),
    ...
    if (!is.null(section_info)) {
      tags$div(
        class = "alert",
        style = paste0("background-color: ", section_info$color, "20; border-left: 4px solid ", section_info$color, "; padding: 10px; margin-bottom: 15px;"),
        section_info$icon,
        tags$strong(section_info$text)
      )
    }
  })
  ...
  # Observers for export navigation buttons
  observeEvent(input$export_de_genes, {
